...
for _, local_name, rtype, note in SOURCE_REPOS:
    ...
    flag = (" [DERIVATION—failures expected]" if rtype == "DERIVATION"
            else " [CUSTOM runner]" if local_name in CUSTOM_RUNNER_REPOS
            else "")
...
#  DERIVATION repos run normally but failures are annotated.
...
for repo_name, res in RUN_VERBOSE.items():
    ...
    if summary.get("failures_expected"):
        ...
        exp_block = [
            ...
            "> This repo is a GR-based derivation/exploration,",
            ...
            "> **Canonical SSZ implementation is `ssz-metric-pure`.**",
